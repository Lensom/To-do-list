window.onload = function() { 
    ...
    function renderTodoList() {
        ...
        for (let val of data.todo) {
            addItemToDOM(val);
        }

        for (let val of data.completed) {
            addItemToDOM(val, true);
        }
    }

    const dataObjectUpdated = () => localStorage.setItem('todoList', JSON.stringify(data));
    ...
    function removeItem() {
        ...
        let separator = (id === 'todo') ? data.todo.splice(data.todo.indexOf(value), 1) : data.completed.splice(data.completed.indexOf(value), 1);
